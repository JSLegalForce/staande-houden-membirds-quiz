// =============================================
// JS Legal Force - Kennisquiz Staande Houden
// =============================================
...
document.getElementById('btn-naar-start').addEventListener('click', () => {
  ...
  document.getElementById('start-welkom').innerHTML = `Welkom, ${voornaam}!<br><span style="font-size:1.3rem;font-weight:600">Staande Houden Kennisquiz</span>`;
  toonScherm('start');
});
...
function toonEinde() {
  ...
  document.getElementById('eind-beoordeling-tekst').textContent = geslaagd
    ? 'Gefeliciteerd! Je hebt de kennisquiz Staande Houden met goed gevolg afgerond.'
...
document.getElementById('btn-certificaat').addEventListener('click', () => {
  ...
  doc.setFillColor(26,42,94); doc.rect(0,0,W,28,'F');
  if (geslaagd) { doc.setFillColor(26,122,74); } else { doc.setFillColor(181,42,42); }
  doc.rect(0,0,8,H,'F');
  doc.setFillColor(26,42,94); doc.rect(0,H-20,W,20,'F');
  doc.setFont('helvetica','bold'); doc.setFontSize(16); doc.setTextColor(255,255,255);
  doc.text('JS Legal Force', W/2, 17, {align:'center'});
  doc.setFont('helvetica','normal'); doc.setFontSize(8); doc.setTextColor(138,154,181);
  doc.text("Kennisquiz Staande Houden - voor BOA's", W/2, 23, {align:'center'});
  doc.setDrawColor(200,210,230); doc.setLineWidth(0.5); doc.rect(14,34,W-22,H-58,'S');
  doc.setLineWidth(0.2); doc.rect(16,36,W-26,H-62,'S');
  ...
  const badgeBreedte = tekstBreedte + 24;
  const badgeX = (W - badgeBreedte) / 2;
  const badgeY = 39; const badgeH = 11;
  if (geslaagd) { doc.setFillColor(230,244,237); doc.setDrawColor(26,122,74); doc.setTextColor(26,122,74); }
  else { doc.setFillColor(250,234,234); doc.setDrawColor(181,42,42); doc.setTextColor(181,42,42); }
  doc.setLineWidth(0.8); doc.roundedRect(badgeX,badgeY,badgeBreedte,badgeH,3,3,'FD');
  doc.text(badgeTekst, W/2, badgeY+(badgeH/2)+1.5, {align:'center'});
  doc.setFont('helvetica','bold'); doc.setFontSize(26); doc.setTextColor(26,42,94);
  doc.text(geslaagd ? 'BEWIJS VAN DEELNAME' : 'RESULTATEN OVERZICHT', W/2, 64, {align:'center'});
  doc.setFont('helvetica','normal'); doc.setFontSize(11); doc.setTextColor(74,85,120);
  doc.text("Kennisquiz Staande Houden voor BOA's", W/2, 72, {align:'center'});
  doc.setDrawColor(200,210,230); doc.setLineWidth(0.6); doc.line(60,77,W-60,77);
  doc.setFont('helvetica','italic'); doc.setFontSize(11); doc.setTextColor(74,85,120);
  doc.text(geslaagd ? 'Dit bewijs van deelname wordt verleend aan' : 'Dit resultatenoverzicht is opgesteld voor', W/2, 87, {align:'center'});
  ...
  doc.setFont('helvetica','bold'); doc.setFontSize(24); doc.setTextColor(26,42,94);
  doc.text(naamTekst, W/2, 99, {align:'center'});
  if (deelnemerOrganisatie) { doc.setFont('helvetica','normal'); doc.setFontSize(11); doc.setTextColor(74,85,120); doc.text(deelnemerOrganisatie.charAt(0).toUpperCase()+deelnemerOrganisatie.slice(1), W/2, 107, {align:'center'}); }
  doc.setDrawColor(200,210,230); doc.setLineWidth(0.5); doc.line(60,113,W-60,113);
  doc.setFont('helvetica','normal'); doc.setFontSize(10.5); doc.setTextColor(26,42,94);
  if (geslaagd) { doc.text('Heeft de kennisquiz Staande Houden van JS Legal Force met goed gevolg afgerond.', W/2, 126, {align:'center'}); }
  else { doc.text('Heeft deelgenomen aan de kennisquiz Staande Houden van JS Legal Force.', W/2, 123, {align:'center'}); doc.text('De vereiste slagingsdrempel is bij deze deelname niet behaald.', W/2, 130, {align:'center'}); }
  ...
  doc.setLineWidth(0.8); doc.roundedRect(scoreX,137,scoreBreedte,19,3,3,'FD');
  ...
  doc.setFont('helvetica','normal'); doc.setFontSize(9.5); doc.setTextColor(74,85,120);
  doc.text(`Datum: ${datum}`, W/2, 162, {align:'center'});
  doc.setFontSize(8); doc.setTextColor(138,154,181);
  ...
  doc.setFont('helvetica','normal'); doc.setFontSize(8); doc.setTextColor(138,154,181);
  doc.text("(c) JS Legal Force - Kennisquiz Staande Houden voor BOA's", W/2, H-10, {align:'center'});
  const prefix = geslaagd ? 'Bewijs_Deelname' : 'Resultaten';
  doc.save(`${prefix}_StaandeHouden_${deelnemerNaam}_${deelnemerAchternaam}_${nu.getFullYear()}.pdf`);
});
...
function genereerNummer() {
  ...
  return `JSLF-SH-${jaar}${maand}${dag}-${rand}`;
}
